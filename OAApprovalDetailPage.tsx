    ...
    return (
        <div className="w-full h-screen bg-slate-50 flex flex-col font-sans">
            ...
            <header className="bg-white px-4 pt-6 pb-4 flex-shrink-0">
                ...
                {/* Subject Visual Header: Submitter <--> Type */}
                <div className="flex items-center justify-center px-4 mb-4">
                    <div className="flex flex-col items-center">
                        <div className="w-14 h-14 bg-[#7dbce3] rounded-full flex items-center justify-center text-white font-bold text-lg border-4 border-white shadow-sm mb-2">
                            {data.submitter.slice(-2)}
                        </div>
                        <p className="text-sm font-black text-slate-800">{data.submitter}</p>
                        <p className="text-[10px] text-slate-400 font-bold">{data.dept}</p>
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm font-black text-slate-800">{data.type}</p>
                        <p className="text-[10px] text-slate-400 font-bold">{data.duration}</p>
                    </div>
                </div>
            </header>
            ...
            <main className="flex-grow overflow-y-auto pt-4 space-y-4 pb-32">
                ...
                <section className="bg-white p-6 shadow-sm border-b border-slate-50">
                    <h3 className="text-base font-black text-slate-900 mb-6">申请信息</h3>
                    <div className="space-y-5">
                        <div className="space-y-1">
                            <p className="text-sm text-slate-400 font-bold">公出时间</p>
                            <p className="text-[15px] font-black text-slate-800">{data.time}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-sm text-slate-400 font-bold">单位</p>
                            <p className="text-[15px] font-black text-slate-800">{data.unit}</p>
                        </div>
                    </div>
                </section>
                ...
                <section className="bg-white py-4 px-6 flex justify-between items-center shadow-sm">
                    <span className="text-base font-black text-slate-600">查看考勤信息</span>
                    <ChevronRightIcon className="text-slate-300 w-5 h-5" />
                </section>

                {/* Approval Flow Section */}
                <section className="bg-white p-6 shadow-sm">
                    <h3 className="text-base font-black text-slate-900 mb-8">审批流程</h3>
                                ...
                                <div key={step.id} className="relative flex items-start pl-8">
                                    ...
                                    <div className="flex-grow">
                                        <div className="flex items-center mb-3">
                                            <h4 className={`text-sm font-black ${step.status === 'completed' ? 'text-[#5fc38f]' : 'text-slate-400'}`}>
                                                {step.role}
                                            </h4>
                                        </div>
                                        ...
                                        <div className={`rounded-xl p-4 ${step.status === 'completed' ? 'bg-[#effbf6]' : 'bg-slate-50'}`}>
                                            <div className="flex justify-between items-start">
                                                ...
                                                {step.time && <span className="text-[10px] text-slate-400 font-bold ml-2">{step.time}</span>}
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-100 p-4 flex space-x-4 z-40 max-w-md mx-auto">
                <button 
                    onClick={onBack}
                    className="flex-1 py-3.5 rounded-full font-black text-lg text-[#f44336] border border-[#f44336] active:bg-red-50 transition-colors"
                >
                    驳回
                </button>
                <button 
                    onClick={onBack}
                    className="flex-grow-[2] py-3.5 rounded-full font-black text-lg text-white bg-[#5fc38f] shadow-lg shadow-[#5fc38f]/20 active:scale-95 transition-all"
                >
                    同意
                </button>
            </footer>
